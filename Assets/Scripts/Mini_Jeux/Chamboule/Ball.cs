...
public class Ball : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        float dist = 0f;
       /* if (ThrowBallManager.Instance.target)
        {
            dist = (ThrowBallManager.Instance.target.position.z - transform.position.z);
        }*/
    ...
    private void isThrow(bool flag)
    {
        ...
        ballThrowed = flag;
        if (ThrowBallManager.Instance.IsGettingDirection)
            isRotate = false;
        else
            isRotate = true;
       // throwPos = (ThrowBallManager.Instance.target.position.z - transform.position.z);
    }
    ...
    IEnumerator JumpAround(float tm)
    {
        ...
        while (!ballThrowed)
        {
            ...
            if (ThrowBallManager.Instance.IsGameStart)
            {
